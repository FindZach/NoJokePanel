...
docker run -d --pull always --name spring-app --network traefik-net -v /var/run/docker.sock:/var/run/docker.sock -l "traefik.enable=true" -l "traefik.http.routers.spring-app.rule=Host(\`gabwiki.com\`) || Host(\`www.gabwiki.com\`)" -l "traefik.http.routers.spring-app.entrypoints=websecure" -l "traefik.http.routers.spring-app.tls.certresolver=myresolver" -l "traefik.http.services.spring-app.loadbalancer.server.port=8080" findzach/nojokepanel:$VERSION || { echo "Spring app failed to start!"; exit 1; }
